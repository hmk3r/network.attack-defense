...
func main() {
	...
	serverAddr, err := snet.ParseUDPAddr(localISD + "-" + localAsn + "," + client.MeowServerIP().String() + ":" + strconv.FormatUint(server.ServerPorts[0], 10))
	if err != nil {
		log.Fatal("Unable to parse server Addr")
	}
	if !*spoof { // non spoofing mode
		...
		if *remote {
			spoofedAddr, err = snet.ParseUDPAddr(client.VictimScionIA + "," + client.VictimIP + ":" + strconv.Itoa(client.VictimPort()))
		} else {
			spoofedAddr, err = snet.ParseUDPAddr(localISD + "-" + localAsn + "," + client.LocalVictimIP().String() + ":" + strconv.Itoa(client.VictimPort()))
		}
